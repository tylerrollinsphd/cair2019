...
##'
##' @examples
##' equitysheet(student_data,"Course","Instructor",
##' "Professor 1","Official_Grade","Race","Gender",10,1,"N")

##' @export
##' @importFrom dplyr mutate group_by filter ungroup min_rank rename select arrange left_join right_join summarise summarize count
##' @importFrom rlang !! enquo
##' @importFrom zip zip zipr
##' @importFrom stringr str_pad
##' @importFrom tidyr complete spread
##' @importFrom extdplyr pct_routine
##' @import flextable
##' @import ggplot2

equitysheet<-function(data,coursecol,instructorcol,instructorid, gradecol,racecol,gendercol,limit,footer,savesheet){
  try({
  if(missing(limit)){
    limit=0
  } else {
...
outfile<-paste0(instructorid," - Equity Report - ",Sys.Date(),".html")
outfile<<-paste0(instructorid," - Equity Report - ",Sys.Date(),".html")
#outfile<<-(ofile)
#print(myft)
#save_as_html(myft,outfile)

  if(savesheet=="Y"){save_as_html(myft,outfile);
      print("Saved to disk");
      print("datafile:myft");
      print(myft)
    }
  else {print("Not saved to disk");
    print("datafile:myft");
    print(myft)
    }
  }, silent = F)
