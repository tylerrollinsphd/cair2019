...
##' @export
##' @importFrom dplyr mutate group_by filter ungroup min_rank
...
gap <- df3 %>%
  ...
  mutate(rank = min_rank(!!-nbr) * 1) %>% mutate(Value = paste(" ",round(!!nbr))) %>% filter(rank<=rankcut)%>%
  ungroup()

p <- ggplot(gap, aes(rank, group = !!Course,
                     fill = as.factor(!!Course), color = as.factor(!!Course))) +
  geom_tile(aes(y = !!nbr/2,
                height = !!nbr,
  ...
  geom_text(aes(rank,y = !!nbr,label = Value, hjust=0)) +
