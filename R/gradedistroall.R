...
##' @examples
##' gradedistroall(student_data,"Course","English 101","Instructor","Professor 1",
##' "Race","Official_Grade","Grading Distribution for English 101",
##' 10,"Race (# students)",0,"N")

##' @export
##' @import ggplot2
##' @importFrom dplyr mutate group_by filter summarise count
...
gradedistroall<-function(data,coursecol,coursename,instructorcol,instructorname,filtercol,gradecol,graphsubtitle,limit,legend,leadingzero,savesheet){
  ...
  if(savesheet=="Y"){ggsave(allgdname,plot=allgdplot,width=11, height = 8,units="in");
    print("Saved to disk");
    print("datafile:allgdplot");
    print(allgdplot)
    }
