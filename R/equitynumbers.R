...
##' @examples
##' equitysheet(testdata,"Course","Instructor","Professor 1",
##' "Official_Grade","Race","Gender",0,1)

##' @export
##' @importFrom dplyr mutate group_by filter ungroup min_rank rename select arrange left_join right_join summarise summarize
...
equitysheetnumbers<-function(data,coursecol,instructorcol,instructorid, gradecol,racecol,gendercol,limit,footer,savesheet){
  try({
    if(missing(limit)){
      limit=0
    } else {
    ...
    outfile1<-paste0(instructorid," - Equity Report Numbers - ",Sys.Date(),".html")
    outfile1<<-paste0(instructorid," - Equity Report Numbers - ",Sys.Date(),".html")


    if(savesheet=="Y"){save_as_html(nmyft,outfile1);print("Saved to disk");print("datafile:nmyft");print(nmyft)
    }
    else {print("Not saved to disk");print(nmyft);print("datafile:nmyft")
    }
  }, silent = F)
}
