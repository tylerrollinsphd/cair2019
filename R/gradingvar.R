...
##' @export
##' @import ggplot2
##' @importFrom dplyr mutate group_by filter select arrange left_join summarise count
##' @importFrom rlang !! enquo
##' @importFrom stringr str_pad str_detect
...
gradevar<-function(data,coursecol,coursename,instructorcol,instructorname,successcol,masterycol,dropcol,indicator,limit,graphtitle,leadingzero, savesheet) {
  ...
  if(savesheet=="Y"){
    ggsave(fname,plot=temp_plot,width=11, height = 8,units="in");
    print("Saved to disk");print("datafile:temp_plot");
    print(temp_plot)
  }
  else {
